...
export default function Dropzone() {
  ...
  const handleUpload = (data: Array<any>): void => {
    setIsHover(false);
    const validFiles = data.filter(file => file.size <= MAX_FILE_SIZE);
    const invalidFiles = data.filter(file => file.size > MAX_FILE_SIZE);
    
    if (invalidFiles.length > 0) {
      toast({
        variant: "destructive",
        title: "File size exceeded",
        description: `${invalidFiles.length} file(s) exceed the 5MB limit and were not added.`,
        duration: 5000,
      });
    }
  ...
  return (
    <ReactDropzone
      ...
      onDropRejected={() => {
        ...
        toast({
          ...
          description: "Allowed Files: Audio, Video and Images. Maximum file size: 5MB.",
          duration: 5000,
        });
      }}
      onError={() => {
    ...
    >
      {({ getRootProps, getInputProps, isDragActive }) => (
        <div
          {...getRootProps()}
          className={`flex items-center justify-center border-2 border-dashed shadow-sm cursor-pointer bg-background h-72 lg:h-80 xl:h-96 rounded-3xl transition-colors duration-200 ease-in-out ${
            isDragActive ? "border-primary bg-primary/10" : "border-secondary hover:border-primary"
          }`}
        >
          <input {...getInputProps()} />
          <div className="space-y-4 text-foreground">
            {isDragActive ? (
              <>
                <div className="flex justify-center text-6xl text-primary">
                  <LuFileSymlink className="animate-bounce" />
                </div>
                <h3 className="text-2xl font-medium text-center">
                  Yes, drop it right here
                </h3>
              </>
            ) : (
              <>
            ...
            <div className="flex justify-center">
              <span className="px-4 py-2 text-sm text-muted-foreground bg-secondary rounded-full">
                Max file size: 5MB
              </span>
            </div>
          </div>
